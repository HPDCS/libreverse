...
static int i386_finalize_immediate(segT, expressionS *, i386_operand_type, const char *);
static int i386_finalize_displacement(segT, expressionS *, i386_operand_type, const char *);
...
static symbolS *pe_lcomm_internal(int needs_align, symbolS * symbolP, addressT size) {
	addressT align = 0;
...
static void pe_lcomm(int needs_align) {
...

void i386_align_code(fragS * fragP, int count) {
	/* Various efficient no-op patterns for aligning code labels.
	   Note: Don't try to assemble the instructions in the comments.
	   0L and 0w are not legal.  */
	static const char f32_1[] = { 0x90 };	/* nop                  */
	static const char f32_2[] = { 0x66, 0x90 };	/* xchg %ax,%ax */
	...
	static const char jump_31[] = { 0xeb, 0x1d, 0x90, 0x90, 0x90, 0x90, 0x90,	/* jmp .+31; lotsa nops */
	...
	if (flag_code == CODE_16BIT) {
		if (count > 8) {
			memcpy(fragP->fr_literal + fragP->fr_fix, jump_31, count);
			/* Adjust jump offset.  */
		...
		if (patt == f32_patt) {
			...
			if (flag_code == CODE_64BIT && fragP->tc_frag_data.isa_flags.bitfield.cpulm)
				limit = 3;
			else
				limit = 15;
			if (count < limit)
			...
			while (padding > 10) {
				padding -= 10;
				memcpy(fragP->fr_literal + fragP->fr_fix + padding, patt[9], 10);
			}

			if (padding)
				memcpy(fragP->fr_literal + fragP->fr_fix, patt[padding - 1], padding);
		}
	}
	fragP->fr_var = count;
}

static INLINE int operand_type_all_zero(const union i386_operand_type *x) {
...
static INLINE void operand_type_set(union i386_operand_type *x, unsigned int v) {
	switch (ARRAY_SIZE(x->array)) {
	case 3:
		x->array[2] = v;
	case 2:
...
static INLINE int operand_type_equal(const union i386_operand_type *x, const union i386_operand_type *y) {
	switch (ARRAY_SIZE(x->array)) {
	case 3:
...
static INLINE int cpu_flags_all_zero(const union i386_cpu_flags *x) {
	switch (ARRAY_SIZE(x->array)) {
	case 3:
		if (x->array[2])
			return 0;
...
static INLINE void cpu_flags_set(union i386_cpu_flags *x, unsigned int v) {
	switch (ARRAY_SIZE(x->array)) {
	case 3:
		x->array[2] = v;
	case 2:
...
static INLINE int cpu_flags_equal(const union i386_cpu_flags *x, const union i386_cpu_flags *y) {
	switch (ARRAY_SIZE(x->array)) {
	case 3:
		if (x->array[2] != y->array[2])
			return 0;
...
static INLINE int cpu_flags_check_cpu64(i386_cpu_flags f) {
	return !((flag_code == CODE_64BIT && f.bitfield.cpuno64)
		 || (flag_code != CODE_64BIT && f.bitfield.cpu64));
}

static INLINE i386_cpu_flags cpu_flags_and(i386_cpu_flags x, i386_cpu_flags y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] &= y.array[2];
	case 2:
...
static INLINE i386_cpu_flags cpu_flags_or(i386_cpu_flags x, i386_cpu_flags y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] |= y.array[2];
	case 2:
...
static INLINE i386_cpu_flags cpu_flags_and_not(i386_cpu_flags x, i386_cpu_flags y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] &= ~y.array[2];
	case 2:
...
static int cpu_flags_match(const insn_template * t) {
			...
			if (x.bitfield.cpuavx) {
				...
				if (cpu.bitfield.cpuavx) {
					...
					if (!t->opcode_modifier.sse2avx || sse2avx) {
						match |= (CPU_FLAGS_ARCH_MATCH | CPU_FLAGS_AVX_MATCH);
						/* Check AES.  */
						if (!x.bitfield.cpuaes || cpu.bitfield.cpuaes)
							match |= CPU_FLAGS_AES_MATCH;
						/* Check PCLMUL.  */
						if (!x.bitfield.cpupclmul || cpu.bitfield.cpupclmul)
...
static INLINE i386_operand_type operand_type_and(i386_operand_type x, i386_operand_type y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] &= y.array[2];
	case 2:
...
static INLINE i386_operand_type operand_type_or(i386_operand_type x, i386_operand_type y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] |= y.array[2];
	case 2:
...
static INLINE i386_operand_type operand_type_xor(i386_operand_type x, i386_operand_type y) {
	switch (ARRAY_SIZE(x.array)) {
	case 3:
		x.array[2] ^= y.array[2];
	case 2:
...
static const i386_operand_type inoutportreg = OPERAND_TYPE_INOUTPORTREG;
static const i386_operand_type reg16_inoutportreg = OPERAND_TYPE_REG16_INOUTPORTREG;
...
static INLINE int operand_type_check(i386_operand_type t, enum operand_type c) {
	switch (c) {
	case reg:
		return (t.bitfield.reg8 || t.bitfield.reg16 || t.bitfield.reg32 || t.bitfield.reg64);

	case imm:
		return (t.bitfield.imm8 || t.bitfield.imm8s || t.bitfield.imm16 || t.bitfield.imm32 || t.bitfield.imm32s || t.bitfield.imm64);

	case disp:
		return (t.bitfield.disp8 || t.bitfield.disp16 || t.bitfield.disp32 || t.bitfield.disp32s || t.bitfield.disp64);

	case anymem:
		return (t.bitfield.disp8 || t.bitfield.disp16 || t.bitfield.disp32 || t.bitfield.disp32s || t.bitfield.disp64 || t.bitfield.baseindex);
...
static INLINE int match_reg_size(const insn_template * t, unsigned int j) {
	return !((i.types[j].bitfield.byte && !t->operand_types[j].bitfield.byte)
		 || (i.types[j].bitfield.word && !t->operand_types[j].bitfield.word)
		 || (i.types[j].bitfield.dword && !t->operand_types[j].bitfield.dword)
		 || (i.types[j].bitfield.qword && !t->operand_types[j].bitfield.qword));
}

/* Return 1 if there is no conflict in any size on operand J for
   instruction template T.  */

static INLINE int match_mem_size(const insn_template * t, unsigned int j) {
	return (match_reg_size(t, j)
		&& !((i.types[j].bitfield.unspecified && !t->operand_types[j].bitfield.unspecified)
		     || (i.types[j].bitfield.fword && !t->operand_types[j].bitfield.fword)
		     || (i.types[j].bitfield.tbyte && !t->operand_types[j].bitfield.tbyte)
		     || (i.types[j].bitfield.xmmword && !t->operand_types[j].bitfield.xmmword)
		     || (i.types[j].bitfield.ymmword && !t->operand_types[j].bitfield.ymmword)
		     || (i.types[j].bitfield.zmmword && !t->operand_types[j].bitfield.zmmword)));
}

/* Return 1 if there is no size conflict on any operands for
   instruction template T.  */

static INLINE int operand_size_match(const insn_template * t) {
	unsigned int j;
	int match = 1;

	/* Don't check jump instructions.  */
	if (t->opcode_modifier.jump || t->opcode_modifier.jumpbyte || t->opcode_modifier.jumpdword || t->opcode_modifier.jumpintersegment)
		return match;
 ...
 mismatch:
	...
	match = 1;
	for (j = 0; j < 2; j++) {
		if (t->operand_types[j].bitfield.acc && !match_reg_size(t, j ? 0 : 1))
			goto mismatch;

		if (i.types[j].bitfield.mem && !match_mem_size(t, j ? 0 : 1))
			goto mismatch;
	}
...
static INLINE int operand_type_match(i386_operand_type overlap, i386_operand_type given) {
	...
	if (given.bitfield.baseindex == overlap.bitfield.baseindex && given.bitfield.jumpabsolute == overlap.bitfield.jumpabsolute)
		return 1;
...
static INLINE int operand_type_register_match(i386_operand_type m0, i386_operand_type g0, i386_operand_type t0, i386_operand_type m1, i386_operand_type g1, i386_operand_type t1) {
	...
	if (g0.bitfield.reg8 == g1.bitfield.reg8 && g0.bitfield.reg16 == g1.bitfield.reg16 && g0.bitfield.reg32 == g1.bitfield.reg32 && g0.bitfield.reg64 == g1.bitfield.reg64)
		return 1;
...
static INLINE unsigned int register_number(const reg_entry * r) {
...
static INLINE unsigned int mode_from_disp_size(i386_operand_type t) {
	if (t.bitfield.disp8 || t.bitfield.vec_disp8)
		return 1;
	else if (t.bitfield.disp16 || t.bitfield.disp32 || t.bitfield.disp32s)
		return 2;
	else
		return 0;
}

static INLINE int fits_in_signed_byte(addressT num) {
	return num + 0x80 <= 0xff;
}

static INLINE int fits_in_unsigned_byte(addressT num) {
	return num <= 0xff;
}

static INLINE int fits_in_unsigned_word(addressT num) {
	return num <= 0xffff;
}

static INLINE int fits_in_signed_word(addressT num) {
	return num + 0x8000 <= 0xffff;
}

static INLINE int fits_in_signed_long(addressT num ATTRIBUTE_UNUSED) {
#ifndef BFD64
	return 1;
#else
...
static INLINE int fits_in_unsigned_long(addressT num ATTRIBUTE_UNUSED) {
#ifndef BFD64
	return 1;
#else
...
static INLINE int fits_in_vec_disp8(offsetT num) {
...
static INLINE int fits_in_imm4(offsetT num) {
	return (num & 0xf) == num;
}

static i386_operand_type smallest_imm_type(offsetT num) {
	i386_operand_type t;
...
static offsetT offset_in_range(offsetT val, int size) {
	addressT mask;
...
static enum PREFIX_GROUP add_prefix(unsigned int prefix) {
	enum PREFIX_GROUP ret = PREFIX_OTHER;
	unsigned int q;

	if (prefix >= REX_OPCODE && prefix < REX_OPCODE + 16 && flag_code == CODE_64BIT) {
...
static void update_code_flag(int value, int check) {
	...
	if (value == CODE_64BIT && !cpu_arch_flags.bitfield.cpulm) {
		if (check)
			...
			as_error = as_bad;
		(*as_error) (_("64bit mode not supported on `%s'."), cpu_arch_name ? cpu_arch_name : default_arch);
	}
	if (value == CODE_32BIT && !cpu_arch_flags.bitfield.cpui386) {
		if (check)
			...
			as_error = as_bad;
		(*as_error) (_("32bit mode not supported on `%s'."), cpu_arch_name ? cpu_arch_name : default_arch);
	}
	stackop_size = '\0';
}

static void set_code_flag(int value) {
	update_code_flag(value, 0);
}

static void set_16bit_gcc_code_flag(int new_code_flag) {
...
static void set_intel_syntax(int syntax_flag) {
	...
	if (ask_naked_reg == 0)
		allow_naked_reg = (intel_syntax && (bfd_get_symbol_leading_char(stdoutput) != '\0'));
	else
...
static void set_intel_mnemonic(int mnemonic_flag) {
	intel_mnemonic = mnemonic_flag;
}

static void set_allow_index_reg(int flag) {
	allow_index_reg = flag;
}

static void set_check(int what) {
...
static void check_cpu_arch_compatible(const char *name ATTRIBUTE_UNUSED, i386_cpu_flags new_flag ATTRIBUTE_UNUSED) {
#if defined (OBJ_ELF) || defined (OBJ_MAYBE_ELF)
	...
	if (get_elf_backend_data(stdoutput)->elf_machine_code != EM_L1OM || new_flag.bitfield.cpul1om)
		return;

	/* If we are targeting Intel K1OM, we must enable it.  */
	if (get_elf_backend_data(stdoutput)->elf_machine_code != EM_K1OM || new_flag.bitfield.cpuk1om)
		return;
...
static void set_cpu_arch(int dummy ATTRIBUTE_UNUSED) {
	SKIP_WHITESPACE();
		...
		for (j = 0; j < ARRAY_SIZE(cpu_arch); j++) {
			if (strcmp(string, cpu_arch[j].name) == 0) {
				...
				if (!cpu_arch[j].negated)
					flags = cpu_flags_or(cpu_arch_flags, cpu_arch[j].flags);
				else
					flags = cpu_flags_and_not(cpu_arch_flags, cpu_arch[j].flags);
				if (!cpu_flags_equal(&flags, &cpu_arch_flags)) {
					if (cpu_sub_arch_name) {
						char *name = cpu_sub_arch_name;
						cpu_sub_arch_name = concat(name, cpu_arch[j].name, (const char *)NULL);
						free(name);
					} else
	...
	if (*input_line_pointer == ',' && !is_end_of_line[(unsigned char)input_line_pointer[1]]) {
		char *string = ++input_line_pointer;
		int e = get_symbol_end();
...
enum bfd_architecture i386_arch(void) {
	if (cpu_arch_isa == PROCESSOR_L1OM) {
		if (OUTPUT_FLAVOR != bfd_target_elf_flavour || flag_code != CODE_64BIT)
...
unsigned long i386_mach(void) {
	if (!strncmp(default_arch, "x86_64", 6)) {
		if (cpu_arch_isa == PROCESSOR_L1OM) {
			if (OUTPUT_FLAVOR != bfd_target_elf_flavour || default_arch[6] != '\0')
				as_fatal(_("Intel L1OM is 64bit ELF only"));
...
void md_begin(void) {
...

	{
		...
		while (1) {
			...
			if (optab->name == NULL || strcmp(optab->name, (optab - 1)->name) != 0) {
				...
				if (hash_err) {
					as_fatal(_("can't hash %s: %s"), (optab - 1)->name, hash_err);
				}
				if (optab->name == NULL)
					break;
				core_optab = (templates *) xmalloc(sizeof(templates));
	...
	reg_hash = hash_new();
	{
		...
		for (regtab = i386_regtab; regtab_size--; regtab++) {
			...
			if (hash_err)
				as_fatal(_("can't hash %s: %s"), regtab->reg_name, hash_err);
		}
	}
	...
	if (flag_code == CODE_64BIT) {
		...
		x86_dwarf2_return_column = (OUTPUT_FLAVOR == bfd_target_coff_flavour ? 32 : 16);
#else
...
void i386_print_statistics(FILE * file) {
...
static void pi(char *line, i386_insn * x) {
	unsigned int j;

	fprintf(stdout, "%s: template ", line);
	pte(&x->tm);
	fprintf(stdout, "  address: base %s  index %s  scale %x\n", x->base_reg ? x->base_reg->reg_name : "none", x->index_reg ? x->index_reg->reg_name : "none", x->log2_scale_factor);
	fprintf(stdout, "  modrm:  mode %x  reg %x  reg/mem %x\n", x->rm.mode, x->rm.reg, x->rm.regmem);
	fprintf(stdout, "  sib:  base %x  index %x  scale %x\n", x->sib.base, x->sib.index, x->sib.scale);
	fprintf(stdout, "  rex: 64bit %x  extX %x  extY %x  extZ %x\n", (x->rex & REX_W) != 0, (x->rex & REX_R) != 0, (x->rex & REX_X) != 0, (x->rex & REX_B) != 0);
	for (j = 0; j < x->operands; j++) {
		fprintf(stdout, "    #%d:  ", j + 1);
		pt(x->types[j]);
		fprintf(stdout, "\n");
		if (x->types[j].bitfield.reg8 || x->types[j].bitfield.reg16 || x->types[j].bitfield.reg32 || x->types[j].bitfield.reg64 || x->types[j].bitfield.regmmx || x->types[j].bitfield.regxmm || x->types[j].bitfield.regymm || x->types[j].bitfield.regzmm || x->types[j].bitfield.sreg2 || x->types[j].bitfield.sreg3 || x->types[j].bitfield.control || x->types[j].bitfield.debug || x->types[j].bitfield.test)
...
static void pte(insn_template * t) {
	unsigned int j;
...
static void ps(symbolS * s) {
	fprintf(stdout, "%s type %s%s", S_GET_NAME(s), S_IS_EXTERNAL(s) ? "EXTERNAL " : "", segment_name(S_GET_SEGMENT(s)));
}
...
static void pt(i386_operand_type t) {
	unsigned int j;
	i386_operand_type a;
...
#endif				/* DEBUG386 */

static bfd_reloc_code_real_type reloc(unsigned int size, int pcrel, int sign, bfd_reloc_code_real_type other) {
	if (other != NO_RELOC) {
		...
		if (!rel)
			...
			as_bad(_("%u-byte relocation cannot be applied to %u-byte field"), bfd_get_reloc_size(rel), size);
		else if (pcrel && !rel->pc_relative)
			as_bad(_("non-pc-relative relocation for pc-relative field"));
		else if ((rel->complain_on_overflow == complain_overflow_signed && !sign)
			 || (rel->complain_on_overflow == complain_overflow_unsigned && sign > 0))
	...
	if (pcrel) {
		...
		as_bad(_("cannot do %s %u byte relocation"), sign > 0 ? "signed" : "unsigned", size);
	}
...
int tc_i386_fix_adjustable(fixS * fixP ATTRIBUTE_UNUSED) {
#if defined (OBJ_ELF) || defined (OBJ_MAYBE_ELF)
	if (!IS_ELF)
		return 1;
	...
	/* Don't adjust pc-relative references to merge sections in 64-bit
	   mode.  */
	if (use_rela_relocations && (S_GET_SEGMENT(fixP->fx_addsy)->flags & SEC_MERGE) != 0 && fixP->fx_pcrel)
		return 0;

	/* The x86_64 GOTPCREL are represented as 32bit PCrel relocations
	   and changed later by validate_fix.  */
	if (GOT_symbol && fixP->fx_subsy == GOT_symbol && fixP->fx_r_type == BFD_RELOC_32_PCREL)
		return 0;
...
static void build_vex_prefix(const insn_template * t) {
	...
	/* Use 2-byte VEX prefix by swappping destination and source
	   operand.  */
	if (!i.swap_operand && i.operands == i.reg_operands && i.tm.opcode_modifier.vexopcode == VEX0F && i.tm.opcode_modifier.s && i.rex == REX_B) {
	...
	if (i.tm.opcode_modifier.vexopcode == VEX0F && i.tm.opcode_modifier.vexw != VEXW1 && (i.rex & (REX_W | REX_X | REX_B)) == 0) {
		/* 2-byte VEX prefix.  */
		unsigned int r;
		...
		/* Check the REX.R bit.  */
		r = (i.rex & REX_R) ? 0 : 1;
		i.vex.bytes[1] = (r << 7 | register_specifier << 3 | vector_length << 2 | implied_prefix);
	} else {
		/* 3-byte VEX prefix.  */
		unsigned int m, w;
		...
		i.vex.bytes[2] = (w << 7 | register_specifier << 3 | vector_length << 2 | implied_prefix);
	}
}

/* Build the EVEX prefix.  */

static void build_evex_prefix(void) {
	unsigned int register_specifier;
	unsigned int implied_prefix;
	unsigned int m, w;
...
static void process_immext(void) {
	expressionS *exp;
	...
	if ((i.tm.cpu_flags.bitfield.cpusse3 || i.tm.cpu_flags.bitfield.cpusvme)
		...
		for (x = 0; x < i.operands; x++)
			if (register_number(i.op[x].regs) != x)
				as_bad(_("can't use register '%s%s' as operand %d in '%s'."), register_prefix, i.op[x].regs->reg_name, x + 1, i.tm.name);
	...
	gas_assert(i.imm_operands <= 1 && (i.operands <= 2 || ((i.tm.opcode_modifier.vex || i.tm.opcode_modifier.evex)
							       && i.operands <= 4)));
...
static int check_hle(void) {
	switch (i.tm.opcode_modifier.hleprefixok) {
	default:
		abort();
	case HLEPrefixNone:
		as_bad(_("invalid instruction `%s' after `%s'"), i.tm.name, i.hle_prefix);
		return 0;
	case HLEPrefixLock:
	...
	case HLEPrefixRelease:
		if (i.prefix[HLE_PREFIX] != XRELEASE_PREFIX_OPCODE) {
			as_bad(_("instruction `%s' after `xacquire' not allowed"), i.tm.name);
			return 0;
		}
		if (i.mem_operands == 0 || !operand_type_check(i.types[i.operands - 1], anymem)) {
			as_bad(_("memory destination needed for instruction `%s'" " after `xrelease'"), i.tm.name);
			return 0;
		}
		return 1;
	}
...
void md_assemble(char *line) {
	...
	if (intel_syntax && i.operands > 1 && (strcmp(mnemonic, "bound") != 0)
	...
	if (i.imm_operands == 2 && (strcmp(mnemonic, "extrq") == 0 || strcmp(mnemonic, "insertq") == 0))
		swap_2_operands(0, 1);
	...
	/* Don't optimize displacement for movabs since it only takes 64bit
	   displacement.  */
	if (i.disp_operands && i.disp_encoding != disp_encoding_32bit && (flag_code != CODE_64BIT || strcmp(mnemonic, "movabs") != 0))
		optimize_disp();
	...
	if (sse_check != check_none && !i.tm.opcode_modifier.noavx && (i.tm.cpu_flags.bitfield.cpusse || i.tm.cpu_flags.bitfield.cpusse2 || i.tm.cpu_flags.bitfield.cpusse3 || i.tm.cpu_flags.bitfield.cpussse3 || i.tm.cpu_flags.bitfield.cpusse4_1 || i.tm.cpu_flags.bitfield.cpusse4_2)) {
		(sse_check == check_warning ? as_warn : as_bad) (_("SSE instruction `%s' is used"), i.tm.name);
	}
	...
	if ((i.tm.base_opcode & ~9) == 0x0fb6) {
		...
		if (i.reg_operands != 2 && !i.suffix && intel_syntax)
	...
	if (i.rep_prefix && !i.tm.opcode_modifier.repprefixok) {
		as_bad(_("invalid instruction `%s' after `%s'"), i.tm.name, i.rep_prefix);
		return;
	}
	...
	if (i.prefix[LOCK_PREFIX]
	    && (!i.tm.opcode_modifier.islockable || i.mem_operands == 0 || (i.tm.base_opcode != 0x86 && !operand_type_check(i.types[i.operands - 1], anymem)))) {
	...
	if (i.tm.cpu_flags.bitfield.cpumpx && flag_code == CODE_64BIT && i.prefix[ADDR_PREFIX])
		as_bad(_("32-bit address isn't allowed in 64-bit MPX instructions."));

	/* Insert BND prefix.  */
	if (add_bnd_prefix && i.tm.opcode_modifier.bndprefixok && !i.prefix[BND_PREFIX])
	...
	if (i.operands <= 3)
		for (j = 0; j < i.operands; j++)
			if (i.types[j].bitfield.inoutportreg || i.types[j].bitfield.shiftcount || i.types[j].bitfield.acc || i.types[j].bitfield.floatacc)
				i.reg_operands--;

	/* ImmExt should be processed after SSE2AVX.  */
	if (!i.tm.opcode_modifier.sse2avx && i.tm.opcode_modifier.immext)
		process_immext();
	...
	if (i.tm.opcode_modifier.vex || i.tm.opcode_modifier.evex) {
		if (flag_code == CODE_16BIT) {
			as_bad(_("instruction `%s' isn't supported in 16-bit mode."), i.tm.name);
			return;
		}
	...
	if (i.tm.base_opcode == INT_OPCODE && !i.tm.opcode_modifier.modrm && i.op[0].imms->X_add_number == 3) {
		i.tm.base_opcode = INT3_OPCODE;
		i.imm_operands = 0;
	}

	if ((i.tm.opcode_modifier.jump || i.tm.opcode_modifier.jumpbyte || i.tm.opcode_modifier.jumpdword)
	...
	if ((i.types[0].bitfield.reg8 && (i.op[0].regs->reg_flags & RegRex64) != 0)
	    || (i.types[1].bitfield.reg8 && (i.op[1].regs->reg_flags & RegRex64) != 0)
	    || ((i.types[0].bitfield.reg8 || i.types[1].bitfield.reg8)
		&& i.rex != 0)) {
		int x;
		...
		for (x = 0; x < 2; x++) {
			...
			if (i.types[x].bitfield.reg8 && (i.op[x].regs->reg_flags & RegRex64) == 0) {
				/* In case it is "hi" register, give up.  */
				if (i.op[x].regs->reg_num > 3)
					as_bad(_("can't encode register '%s%s' in an " "instruction requiring REX prefix."), register_prefix, i.op[x].regs->reg_name);
...
static char *parse_insn(char *line, char *mnemonic) {
	...
	while (1) {
		...
		if (!is_space_char(*l)
		    && *l != END_OF_INSN && (intel_syntax || (*l != PREFIX_SEPARATOR && *l != ','))) {
		...
		if (*l != END_OF_INSN && (!is_space_char(*l) || l[1] != END_OF_INSN)
		    && current_templates && current_templates->start->opcode_modifier.isprefix) {
			if (!cpu_flags_check_cpu64(current_templates->start->cpu_flags)) {
				as_bad((flag_code != CODE_64BIT ? _("`%s' is only supported in 64-bit mode")
					: _("`%s' is not supported in 64-bit mode")), current_templates->start->name);
				return NULL;
			}
			/* If we are in 16-bit mode, do not allow addr16 or data16.
			   Similarly, in 32-bit mode, do not allow addr32 or data32.  */
			if ((current_templates->start->opcode_modifier.size16 || current_templates->start->opcode_modifier.size32)
			    && flag_code != CODE_64BIT && (current_templates->start->opcode_modifier.size32 ^ (flag_code == CODE_16BIT))) {
	...
	if (!current_templates) {
		...
		if (mnem_p - 2 == dot_p && dot_p[1] == 's')
			i.swap_operand = 1;
		else if (mnem_p - 3 == dot_p && dot_p[1] == 'd' && dot_p[2] == '8')
 ...
 check_suffix:
	...
	if (!(supported & CPU_FLAGS_64BIT_MATCH)) {
		as_bad(flag_code == CODE_64BIT ? _("`%s' is not supported in 64-bit mode")
		       : _("`%s' is only supported in 64-bit mode"), current_templates->start->name);
		return NULL;
	}
	if (supported != CPU_FLAGS_PERFECT_MATCH) {
		as_bad(_("`%s' is not supported on `%s%s'"), current_templates->start->name, cpu_arch_name ? cpu_arch_name : default_arch, cpu_sub_arch_name ? cpu_sub_arch_name : "");
		return NULL;
	}
...
static char *parse_operands(char *l, const char *mnemonic) {
	...
	while (*l != END_OF_INSN) {
		...
		if (!is_operand_char(*l) && *l != END_OF_INSN) {
			as_bad(_("invalid character %s before operand %d"), output_invalid(*l), i.operands + 1);
			return NULL;
		}
		...
		while (paren_not_balanced || *l != ',') {
			if (*l == END_OF_INSN) {
				...
				as_bad(_("invalid character %s in operand %d"), output_invalid(*l), i.operands + 1);
				return NULL;
			}
			if (!intel_syntax) {
				if (*l == '(')
...
static void swap_2_operands(int xchg1, int xchg2) {
	union i386_op temp_op;
	i386_operand_type temp_type;
		...
		if (operand_type_check(i.types[op], imm)) {
			...
			case O_constant:
				...
				if ((i.types[op].bitfield.imm16)
					...
					i.op[op].imms->X_add_number = (((i.op[op].imms->X_add_number & 0xffff) ^ 0x8000) - 0x8000);
				}
				if ((i.types[op].bitfield.imm32)
				    && ((i.op[op].imms->X_add_number & ~(((offsetT) 2 << 31) - 1))
					== 0)) {
					i.op[op].imms->X_add_number = ((i.op[op].imms->X_add_number ^ ((offsetT) 1 << 31))
								       - ((offsetT) 1 << 31));
				}
				i.types[op]
				    = operand_type_or(i.types[op], smallest_imm_type(i.op[op].imms->X_add_number));
	...
	for (op = i.operands; --op >= 0;)
		if (operand_type_check(i.types[op], disp)) {
			if (i.op[op].disps->X_op == O_constant) {
				...
				if (i.types[op].bitfield.disp16 && (op_disp & ~(offsetT) 0xffff) == 0) {
				...
				if (i.types[op].bitfield.disp32 && (op_disp & ~(((offsetT) 2 << 31) - 1)) == 0) {
				...
				if ((i.types[op].bitfield.disp32 || i.types[op].bitfield.disp32s || i.types[op].bitfield.disp16)
				    && fits_in_signed_byte(op_disp))
					i.types[op].bitfield.disp8 = 1;
			} else if (i.reloc[op] == BFD_RELOC_386_TLS_DESC_CALL || i.reloc[op] == BFD_RELOC_X86_64_TLSDESC_CALL) {
				fix_new_exp(frag_now, frag_more(0) - frag_now->fr_literal, 0, i.op[op].disps, 0, i.reloc[op]);
				i.types[op].bitfield.disp8 = 0;
				i.types[op].bitfield.disp16 = 0;
				i.types[op].bitfield.disp32 = 0;
				i.types[op].bitfield.disp32s = 0;
...
static int check_VecOperands(const insn_template * t) {
	unsigned int op;

	/* Without VSIB byte, we can't have a vector register for index.  */
	if (!t->opcode_modifier.vecsib && i.index_reg && (i.index_reg->reg_type.bitfield.regxmm || i.index_reg->reg_type.bitfield.regymm || i.index_reg->reg_type.bitfield.regzmm)) {
		i.error = unsupported_vector_index_register;
		return 1;
	}

	/* Check if default mask is allowed.  */
	if (t->opcode_modifier.nodefmask && (!i.mask || i.mask->mask->reg_num == 0)) {
	...
	if (t->opcode_modifier.vecsib) {
		if (!i.index_reg || !((t->opcode_modifier.vecsib == VecSIB128 && i.index_reg->reg_type.bitfield.regxmm)
				      || (t->opcode_modifier.vecsib == VecSIB256 && i.index_reg->reg_type.bitfield.regymm)
				      || (t->opcode_modifier.vecsib == VecSIB512 && i.index_reg->reg_type.bitfield.regzmm))) {
		...
		if (i.reg_operands == 2 && !i.mask) {
			gas_assert(i.types[0].bitfield.regxmm || i.types[0].bitfield.regymm);
			gas_assert(i.types[2].bitfield.regxmm || i.types[2].bitfield.regymm);
			...
			if ((i.types[1].bitfield.regymm || i.types[1].bitfield.regzmm)
	...
	if (i.broadcast) {
		...
		if (i.broadcast->type != t->opcode_modifier.broadcast || !i.types[i.broadcast->operand].bitfield.mem || (t->opcode_modifier.vecesize == 0 && !i.types[i.broadcast->operand].bitfield.dword && !i.types[i.broadcast->operand].bitfield.unspecified)
		    || (t->opcode_modifier.vecesize == 1 && !i.types[i.broadcast->operand].bitfield.qword && !i.types[i.broadcast->operand].bitfield.unspecified))
			goto bad_broadcast;
		...
		if ((broadcasted_opnd_size == 256 && !t->operand_types[i.broadcast->operand].bitfield.ymmword)
		    || (broadcasted_opnd_size == 512 && !t->operand_types[i.broadcast->operand].bitfield.zmmword)) {
 bad_broadcast:
	...
	/* If broadcast is supported in this instruction, we need to check if
	   operand of one-element size isn't specified without broadcast.  */
		...
		if ((t->opcode_modifier.vecesize == 0 && i.types[op].bitfield.dword)
		    || (t->opcode_modifier.vecesize == 1 && i.types[op].bitfield.qword)) {
	...
	if (i.mask && (!t->opcode_modifier.masking || (i.mask->zeroing && t->opcode_modifier.masking == MERGING_MASKING))) {
	...
	/* Check RC/SAE.  */
	if (i.rounding) {
		if ((i.rounding->type != saeonly && !t->opcode_modifier.staticrounding)
		    || (i.rounding->type == saeonly && (t->opcode_modifier.staticrounding || !t->opcode_modifier.sae))) {
		...
		/* If the instruction has several immediate operands and one of
		   ...
		   immediate operand.  */
		if (i.imm_operands > 1 && i.rounding->operand != (int)(i.imm_operands - 1)) {
...
static int VEX_check_operands(const insn_template * t) {
	...
	if (t->operand_types[0].bitfield.vec_imm4) {
		if (i.op[0].imms->X_op != O_constant || !fits_in_imm4(i.op[0].imms->X_add_number)) {
			i.error = bad_imm4;
			return 1;
		}
...
#error "MAX_OPERANDS must be 5."
	...
	for (t = current_templates->start; t < current_templates->end; t++) {
		...
		if (i.suffix == QWORD_MNEM_SUFFIX && flag_code != CODE_64BIT && (intel_syntax ? (!t->opcode_modifier.ignoresize && !intel_float_operand(t->name))
										 : intel_float_operand(t->name) != 2)
		    && ((!operand_types[0].bitfield.regmmx && !operand_types[0].bitfield.regxmm && !operand_types[0].bitfield.regymm && !operand_types[0].bitfield.regzmm)
			|| (!operand_types[t->operands > 1].bitfield.regmmx && operand_types[t->operands > 1].bitfield.regxmm && operand_types[t->operands > 1].bitfield.regymm && operand_types[t->operands > 1].bitfield.regzmm))
		    && (t->base_opcode != 0x0fc7 || t->extension_opcode != 1 /* cmpxchg8b */ ))
			continue;

		/* In general, don't allow 32-bit operands on pre-386.  */
		else if (i.suffix == LONG_MNEM_SUFFIX && !cpu_arch_flags.bitfield.cpui386 && (intel_syntax ? (!t->opcode_modifier.ignoresize && !intel_float_operand(t->name))
											      : intel_float_operand(t->name) != 2)
			 && ((!operand_types[0].bitfield.regmmx && !operand_types[0].bitfield.regxmm)
			     || (!operand_types[t->operands > 1].bitfield.regmmx && operand_types[t->operands > 1].bitfield.regxmm)))
			continue;
		...
		case 2:
			...
			if (flag_code == CODE_64BIT && t->base_opcode == 0x90 && operand_type_equal(&i.types[0], &acc32)
		...
		case 5:
			...
			if (!operand_type_match(overlap0, i.types[0])
			    ...
			    || (check_register && !operand_type_register_match(overlap0, i.types[0], operand_types[0], overlap1, i.types[1], operand_types[1]))) {
 ...
 check_reverse:
				...
				if (!operand_type_match(overlap0, i.types[0])
				    || !operand_type_match(overlap1, i.types[1])
				    || (check_register && !operand_type_register_match(overlap0, i.types[0], operand_types[1], overlap1, i.types[1], operand_types[0]))) {
				...
				case 5:
					if (!operand_type_match(overlap4, i.types[4])
					    || !operand_type_register_match(overlap3, i.types[3], operand_types[3], overlap4, i.types[4], operand_types[4]))
						continue;
				case 4:
					if (!operand_type_match(overlap3, i.types[3])
					    || (check_register && !operand_type_register_match(overlap2, i.types[2], operand_types[2], overlap3, i.types[3], operand_types[3])))
						continue;
				case 3:
					...
					if (!operand_type_match(overlap2, i.types[2])
					    || (check_register && !operand_type_register_match(overlap1, i.types[1], operand_types[1], overlap2, i.types[2], operand_types[2])))
						continue;
					break;
				}
			}
	...
	if (t == current_templates->end) {
		...
		case unsupported_syntax:
			...
			break;
		case unsupported:
			as_bad(_("unsupported instruction `%s'"), current_templates->start->name);
			return NULL;
		...
		as_bad(_("%s for `%s'"), err_msg, current_templates->start->name);
		return NULL;
	}

	if (!quiet_warnings) {
		if (!intel_syntax && (i.types[0].bitfield.jumpabsolute != operand_types[0].bitfield.jumpabsolute)) {
			as_warn(_("indirect %s without `*'"), t->name);
		}

		if (t->opcode_modifier.isprefix && t->opcode_modifier.ignoresize) {
...
static int check_string(void) {
	...
	if (i.tm.operand_types[mem_op].bitfield.esseg) {
		if (i.seg[0] != NULL && i.seg[0] != &es) {
			as_bad(_("`%s' operand %d must use `%ses' segment"), i.tm.name, mem_op + 1, register_prefix);
			return 0;
		}
		...
		if (i.seg[1] != NULL && i.seg[1] != &es) {
			as_bad(_("`%s' operand %d must use `%ses' segment"), i.tm.name, mem_op + 2, register_prefix);
			return 0;
		}
	}
	return 1;
...
static int process_suffix(void) {
	...
	if (i.tm.opcode_modifier.size16)
		...
		if (!i.suffix) {
			...
			if (!i.suffix) {
				...
				if (i.tm.base_opcode == 0xf20f38f1 || i.tm.base_opcode == 0xf20f38f0) {
					/* We have to know the operand size for crc32.  */
					as_bad(_("ambiguous memory operand size for `%s`"), i.tm.name);
					return 0;
				}
			...
			if (intel_syntax && i.tm.opcode_modifier.ignoresize && i.tm.opcode_modifier.no_bsuf)
				i.suffix = 0;
			else if (!check_byte_reg())
				return 0;
		} else if (i.suffix == LONG_MNEM_SUFFIX) {
			if (intel_syntax && i.tm.opcode_modifier.ignoresize && i.tm.opcode_modifier.no_lsuf)
				i.suffix = 0;
			else if (!check_long_reg())
				return 0;
		} else if (i.suffix == QWORD_MNEM_SUFFIX) {
			if (intel_syntax && i.tm.opcode_modifier.ignoresize && i.tm.opcode_modifier.no_qsuf)
				i.suffix = 0;
			else if (!check_qword_reg())
				return 0;
		} else if (i.suffix == WORD_MNEM_SUFFIX) {
			if (intel_syntax && i.tm.opcode_modifier.ignoresize && i.tm.opcode_modifier.no_wsuf)
				i.suffix = 0;
	...
	} else if (intel_syntax && !i.suffix && (i.tm.operand_types[0].bitfield.jumpabsolute || i.tm.opcode_modifier.jumpbyte || i.tm.opcode_modifier.jumpintersegment || (i.tm.base_opcode == 0x0f01	/* [ls][gi]dt */
	...
	if (!i.suffix) {
		if (!intel_syntax) {
			...
			if (i.tm.opcode_modifier.w || ((suffixes & (suffixes - 1))
						       && !i.tm.opcode_modifier.defaultsize && !i.tm.opcode_modifier.ignoresize)) {
				as_bad(_("ambiguous operand size for `%s'"), i.tm.name);
				return 0;
			}
		}
	...
	if (i.suffix && i.suffix != BYTE_MNEM_SUFFIX && i.suffix != XMMWORD_MNEM_SUFFIX && i.suffix != YMMWORD_MNEM_SUFFIX && i.suffix != ZMMWORD_MNEM_SUFFIX) {
		...
		if (i.tm.opcode_modifier.addrprefixop0) {
			/* The address size override prefix changes the size of the
			   first operand.  */
			if ((flag_code == CODE_32BIT && i.op->regs[0].reg_type.bitfield.reg16)
			    || (flag_code != CODE_32BIT && i.op->regs[0].reg_type.bitfield.reg32))
				if (!add_prefix(ADDR_PREFIX_OPCODE))
					return 0;
		} else if (i.suffix != QWORD_MNEM_SUFFIX && i.suffix != LONG_DOUBLE_MNEM_SUFFIX && !i.tm.opcode_modifier.ignoresize && !i.tm.opcode_modifier.floatmf && ((i.suffix == LONG_MNEM_SUFFIX) == (flag_code == CODE_16BIT)
																					 || (flag_code == CODE_64BIT && i.tm.opcode_modifier.jumpbyte))) {
		...
		if (i.suffix == QWORD_MNEM_SUFFIX && flag_code == CODE_64BIT && !i.tm.opcode_modifier.norex64) {
			/* Special case for xchg %rax,%rax.  It is NOP and doesn't
			   need rex64.  cmpxchg8b is also a special case. */
			if (!(i.operands == 2 && i.tm.base_opcode == 0x90 && i.tm.extension_opcode == None && operand_type_equal(&i.types[0], &acc64)
			      && operand_type_equal(&i.types[1], &acc64))
			    && !(i.operands == 1 && i.tm.base_opcode == 0xfc7 && i.tm.extension_opcode == 1 && !operand_type_check(i.types[0], reg)
...
static int check_byte_reg(void) {
	...
	for (op = i.operands; --op >= 0;) {
		...
		if ((i.types[op].bitfield.reg16 || i.types[op].bitfield.reg32 || i.types[op].bitfield.reg64)
			...
			if (!quiet_warnings)
				as_warn(_("using `%s%s' instead of `%s%s' due to `%c' suffix"), register_prefix, (i.op[op].regs + (i.types[op].bitfield.reg16 ? REGNAM_AL - REGNAM_AX : REGNAM_AL - REGNAM_EAX))->reg_name, register_prefix, i.op[op].regs->reg_name, i.suffix);
#endif
			continue;
		}
		/* Any other register is bad.  */
		if (i.types[op].bitfield.reg16 || i.types[op].bitfield.reg32 || i.types[op].bitfield.reg64 || i.types[op].bitfield.regmmx || i.types[op].bitfield.regxmm || i.types[op].bitfield.regymm || i.types[op].bitfield.regzmm || i.types[op].bitfield.sreg2 || i.types[op].bitfield.sreg3 || i.types[op].bitfield.control || i.types[op].bitfield.debug || i.types[op].bitfield.test || i.types[op].bitfield.floatreg || i.types[op].bitfield.floatacc) {
			as_bad(_("`%s%s' not allowed with `%s%c'"), register_prefix, i.op[op].regs->reg_name, i.tm.name, i.suffix);
			return 0;
		}
	}
	return 1;
...
static int check_long_reg(void) {
	...
	for (op = i.operands; --op >= 0;)
		/* Reject eight bit registers, except where the template requires
		   them. (eg. movzb)  */
		if (i.types[op].bitfield.reg8 && (i.tm.operand_types[op].bitfield.reg16 || i.tm.operand_types[op].bitfield.reg32 || i.tm.operand_types[op].bitfield.acc)) {
			as_bad(_("`%s%s' not allowed with `%s%c'"), register_prefix, i.op[op].regs->reg_name, i.tm.name, i.suffix);
			return 0;
		}
	/* Warn if the e prefix on a general reg is missing.  */
		else if ((!quiet_warnings || flag_code == CODE_64BIT)
			 && i.types[op].bitfield.reg16 && (i.tm.operand_types[op].bitfield.reg32 || i.tm.operand_types[op].bitfield.acc)) {
			/* Prohibit these changes in the 64bit mode, since the
			   lowering is more complicated.  */
			if (flag_code == CODE_64BIT) {
				as_bad(_("incorrect register `%s%s' used with `%c' suffix"), register_prefix, i.op[op].regs->reg_name, i.suffix);
				return 0;
			}
#if REGISTER_WARNINGS
			as_warn(_("using `%s%s' instead of `%s%s' due to `%c' suffix"), register_prefix, (i.op[op].regs + REGNAM_EAX - REGNAM_AX)->reg_name, register_prefix, i.op[op].regs->reg_name, i.suffix);
#endif
		}
	/* Warn if the r prefix on a general reg is present.  */
		else if (i.types[op].bitfield.reg64 && (i.tm.operand_types[op].bitfield.reg32 || i.tm.operand_types[op].bitfield.acc)) {
			if (intel_syntax && i.tm.opcode_modifier.toqword && !i.types[0].bitfield.regxmm) {
...
static int check_qword_reg(void) {
	...
	/* Warn if the r prefix on a general reg is missing.  */
		else if ((i.types[op].bitfield.reg16 || i.types[op].bitfield.reg32)
			 && (i.tm.operand_types[op].bitfield.reg32 || i.tm.operand_types[op].bitfield.acc)) {
			/* Prohibit these changes in the 64bit mode, since the
			   lowering is more complicated.  */
			if (intel_syntax && i.tm.opcode_modifier.todword && !i.types[0].bitfield.regxmm) {
...
static int check_word_reg(void) {
	...
	/* Warn if the e or r prefix on a general reg is present.  */
			 ...
			 && (i.types[op].bitfield.reg32 || i.types[op].bitfield.reg64)
			 && (i.tm.operand_types[op].bitfield.reg16 || i.tm.operand_types[op].bitfield.acc)) {
			/* Prohibit these changes in the 64bit mode, since the
			   lowering is more complicated.  */
			if (flag_code == CODE_64BIT) {
				as_bad(_("incorrect register `%s%s' used with `%c' suffix"), register_prefix, i.op[op].regs->reg_name, i.suffix);
...
#if REGISTER_WARNINGS
			as_warn(_("using `%s%s' instead of `%s%s' due to `%c' suffix"), register_prefix, (i.op[op].regs + REGNAM_AX - REGNAM_EAX)->reg_name, register_prefix, i.op[op].regs->reg_name, i.suffix);
#endif
		}
	return 1;
}

static int update_imm(unsigned int j) {
	i386_operand_type overlap = i.types[j];
	if ((overlap.bitfield.imm8 || overlap.bitfield.imm8s || overlap.bitfield.imm16 || overlap.bitfield.imm32 || overlap.bitfield.imm32s || overlap.bitfield.imm64)
...
static int bad_implicit_operand(int xmm) {
	const char *ireg = xmm ? "xmm0" : "ymm0";

	if (intel_syntax)
		as_bad(_("the last operand of `%s' must be `%s%s'"), i.tm.name, register_prefix, ireg);
	else
		as_bad(_("the first operand of `%s' must be `%s%s'"), i.tm.name, register_prefix, ireg);
	return 0;
}
...
static int process_operands(void) {
	...
	if (i.tm.opcode_modifier.sse2avx && i.tm.opcode_modifier.vexvvvv) {
		...
		gas_assert(i.reg_operands && MAX_OPERANDS > dupl && operand_type_equal(&i.types[dest], &regxmm));
		...
		if (i.tm.opcode_modifier.firstxmm0) {
			...
			gas_assert((MAX_OPERANDS - 1) > dupl && (i.tm.opcode_modifier.vexsources == VEX3SOURCES));
			...
			i.op[0].regs = (const reg_entry *)hash_find(reg_hash, "xmm0");
			i.types[0] = regxmm;
 ...
 duplicate:
		...
		gas_assert(i.reg_operands && (operand_type_equal(&i.types[0], &regxmm)
		...
		gas_assert(i.reg_operands == 1 && i.op[first_reg_op + 1].regs == 0);
	...
	if (i.tm.opcode_modifier.shortform) {
		if (i.types[0].bitfield.sreg2 || i.types[0].bitfield.sreg3) {
			...
			if (i.types[0].bitfield.floatreg || operand_type_check(i.types[0], reg))
				op = 0;
			else
				op = 1;
			...
			if (!quiet_warnings && i.tm.opcode_modifier.ugh) {
				...
				if (i.operands == 2) {
					...
					as_warn(_("translating to `%s %s%s,%s%s'"), i.tm.name, register_prefix, i.op[!intel_syntax].regs->reg_name, register_prefix, i.op[intel_syntax].regs->reg_name);
				} else {
					/* Extraneous `l' suffix on fp insn.  */
					as_warn(_("translating to `%s %s%s'"), i.tm.name, register_prefix, i.op[0].regs->reg_name);
				}
			}
		}
...
static const seg_entry *build_modrm_byte(void) {
	...
	if (vex_3_sources) {
		...
		if (i.tm.opcode_modifier.veximmext && i.tm.opcode_modifier.immext) {
			dest = i.operands - 2;
			gas_assert(dest == 3);
		} else
			dest = i.operands - 1;
		...
		gas_assert((i.reg_operands == 4 || (i.reg_operands == 3 && i.mem_operands == 1))
			   && i.tm.opcode_modifier.vexvvvv == VEXXDS && (i.tm.opcode_modifier.veximmext || (i.imm_operands == 1 && i.types[0].bitfield.vec_imm4 && (i.tm.opcode_modifier.vexw == VEXW0 || i.tm.opcode_modifier.vexw == VEXW1)
		...
		if (i.imm_operands == 0) {
			...
			gas_assert(operand_type_equal(&i.tm.operand_types[reg_slot], &regxmm)
				   || operand_type_equal(&i.tm.operand_types[reg_slot], &regymm)
				   || operand_type_equal(&i.tm.operand_types[reg_slot], &regzmm));
			exp->X_op = O_constant;
			...
			gas_assert(operand_type_equal(&i.tm.operand_types[reg_slot], &regxmm)
				   || operand_type_equal(&i.tm.operand_types[reg_slot], &regymm)
				   || operand_type_equal(&i.tm.operand_types[reg_slot], &regzmm));
			i.op[imm_slot].imms->X_add_number |= register_number(i.op[reg_slot].regs) << 4;
			gas_assert((i.op[reg_slot].regs->reg_flags & RegVRex) == 0);
		}

		gas_assert(operand_type_equal(&i.tm.operand_types[nds], &regxmm)
			   || operand_type_equal(&i.tm.operand_types[nds], &regymm)
			   || operand_type_equal(&i.tm.operand_types[nds], &regzmm));
	...
	if (i.mem_operands == 0 && ((i.reg_operands == 2 && i.tm.opcode_modifier.vexvvvv <= VEXXDS)
				    || (i.reg_operands == 3 && i.tm.opcode_modifier.vexvvvv == VEXXDS)
		...
		case 3:
			...
			gas_assert(i.imm_operands == 1 || (i.imm_operands == 0 && (i.tm.opcode_modifier.vexvvvv == VEXXDS || i.types[0].bitfield.shiftcount)));
		...
		case 4:
			...
			gas_assert((i.imm_operands == 2 && i.types[0].bitfield.imm8 && i.types[1].bitfield.imm8)
				   || (i.tm.opcode_modifier.vexvvvv == VEXXDS && i.imm_operands == 1 && (i.types[0].bitfield.imm8 || i.types[i.operands - 1].bitfield.imm8 || i.rounding)));
			if (i.imm_operands == 2)
				source = 2;
			else {
		...
		case 5:
			if (i.tm.opcode_modifier.evex) {
				...
				gas_assert(i.imm_operands == 2 && i.tm.opcode_modifier.sae && operand_type_check(i.types[0], imm));
		...
		if (!vex_3_sources) {
			...
			if (i.tm.opcode_modifier.vexvvvv == VEXXDS) {
				...
				if (!i.tm.operand_types[source].bitfield.baseindex && i.tm.operand_types[dest].bitfield.baseindex) {
					vvvv = source;
					source = dest;
				} else
					vvvv = dest;
				...
				if ((dest + 1) >= i.operands || (!op.bitfield.reg32 && op.bitfield.reg64 && !operand_type_equal(&op, &regxmm)
		...
		if (!i.tm.operand_types[dest].bitfield.regmem && operand_type_check(i.tm.operand_types[dest], anymem) == 0) {
		...
		if (flag_code != CODE_64BIT && (i.rex & (REX_R | REX_B))) {
			if (!i.types[0].bitfield.control && !i.types[1].bitfield.control)
				abort();
			i.rex &= ~(REX_R | REX_B);
		...
		if (i.mem_operands) {
			...
			if (i.tm.opcode_modifier.vecsib) {
				if (i.index_reg->reg_num == RegEiz || i.index_reg->reg_num == RegRiz)
					abort();
			...
			/* RIP addressing for 64bit mode.  */
			else if (i.base_reg->reg_num == RegRip || i.base_reg->reg_num == RegEip) {
				gas_assert(!i.tm.opcode_modifier.vecsib);
				i.rm.regmem = NO_BASE_REGISTER;
				i.types[op].bitfield.disp8 = 0;
				i.types[op].bitfield.disp16 = 0;
				...
				if (flag_code == CODE_64BIT && operand_type_check(i.types[op], disp)) {
					i386_operand_type temp;
					operand_type_set(&temp, 0);
					temp.bitfield.disp8 = i.types[op].bitfield.disp8;
					temp.bitfield.vec_disp8 = i.types[op].bitfield.vec_disp8;
					i.types[op] = temp;
					if (i.prefix[ADDR_PREFIX] == 0)
						i.types[op].bitfield.disp32s = 1;
					else
				...
				if (!(i.base_reg->reg_flags & RegRex)
				    && (i.base_reg->reg_num == EBP_REG_NUM || i.base_reg->reg_num == ESP_REG_NUM))
					default_seg = &ss;
				...
				if (i.disp_operands && (i.reloc[op] == BFD_RELOC_386_TLS_DESC_CALL || i.reloc[op] == BFD_RELOC_X86_64_TLSDESC_CALL))
					i.rm.mode = 0;
				else {
					if (!fake_zero_displacement && !i.disp_operands && i.disp_encoding) {
						fake_zero_displacement = 1;
		...
		/* Fill in i.rm.reg or i.rm.regmem field with register operand
		   ...
		   registers are coded into the i.rm.reg field.  */
			...
			for (op = 0; op < i.operands; op++)
				if (i.types[op].bitfield.reg8 || i.types[op].bitfield.reg16 || i.types[op].bitfield.reg32 || i.types[op].bitfield.reg64 || i.types[op].bitfield.regmmx || i.types[op].bitfield.regxmm || i.types[op].bitfield.regymm || i.types[op].bitfield.regbnd || i.types[op].bitfield.regzmm || i.types[op].bitfield.regmask || i.types[op].bitfield.sreg2 || i.types[op].bitfield.sreg3 || i.types[op].bitfield.control || i.types[op].bitfield.debug || i.types[op].bitfield.test)
					break;
			...
			if (vex_3_sources)
				...
				if (op > mem) {
					...
					/* Check register-only source operand when two source
					   operands are swapped.  */
					if (!i.tm.operand_types[op].bitfield.baseindex && i.tm.operand_types[op + 1].bitfield.baseindex) {
						vex_reg = op;
						op += 2;
			...
			if (vex_reg != (unsigned int)~0) {
				...
				if (type->bitfield.reg32 != 1 && type->bitfield.reg64 != 1 && !operand_type_equal(type, &regxmm)
...
static void output_branch(void) {
	char *p;
	int size;
	int code16;
	int prefix;
	...
	if (i.prefix[SEG_PREFIX] == CS_PREFIX_OPCODE || i.prefix[SEG_PREFIX] == DS_PREFIX_OPCODE)
...
static void output_jump(void) {
	...
	fixP = fix_new_exp(frag_now, p - frag_now->fr_literal, size, i.op[0].disps, 1, reloc(size, 1, 1, i.reloc[0]));
...
static void output_interseg_jump(void) {
	char *p;
	int size;
	int prefix;
	int code16;
	...
	*p++ = i.tm.base_opcode;
	if (i.op[1].imms->X_op == O_constant) {
		...
		fix_new_exp(frag_now, p - frag_now->fr_literal, size, i.op[1].imms, 0, reloc(size, 0, 0, i.reloc[1]));
...
static void output_insn(void) {
	...
	if (i.tm.opcode_modifier.jump)
		output_branch();
	else if (i.tm.opcode_modifier.jumpbyte || i.tm.opcode_modifier.jumpdword)
		output_jump();
 ...
 check_prefix:
						if (prefix != REPE_PREFIX_OPCODE || (i.prefix[REP_PREFIX]
										     != REPE_PREFIX_OPCODE))
							add_prefix(prefix);
					} else
						add_prefix(prefix);
			...
			if (x86_elf_abi == X86_64_X32_ABI && i.operands == 2 && i.reloc[0] == BFD_RELOC_X86_64_GOTTPOFF && i.prefix[REX_PREFIX] == 0)
		...
		if (i.tm.opcode_modifier.modrm) {
			...
			if (i.rm.regmem == ESCAPE_TO_TWO_BYTE_ADDRESSING && i.rm.mode != 3 && !(i.base_reg && i.base_reg->reg_type.bitfield.reg16))
...
static int disp_size(unsigned int n) {
	int size = 4;
...
static void output_disp(fragS * insn_start_frag, offsetT insn_start_off) {
	...
	for (n = 0; n < i.operands; n++) {
		if (i.types[n].bitfield.vec_disp8 || operand_type_check(i.types[n], disp)) {
			if (i.op[n].disps->X_op == O_constant) {
				...
				if (GOT_symbol && GOT_symbol == i.op[n].disps->X_add_symbol && (((reloc_type == BFD_RELOC_32 || reloc_type == BFD_RELOC_X86_64_32S || (reloc_type == BFD_RELOC_64 && object_64bit))
												 && (i.op[n].disps->X_op == O_symbol || (i.op[n].disps->X_op == O_add && ((symbol_get_value_expression(i.op[n].disps->X_op_symbol)->X_op)
																					  == O_subtract))))
					...
					if (insn_start_frag == frag_now)
						...
						for (fr = insn_start_frag->fr_next; fr && fr != frag_now; fr = fr->fr_next)
							add += fr->fr_fix;
...
static void output_imm(fragS * insn_start_frag, offsetT insn_start_off) {
	char *p;
	unsigned int n;
	...
	for (n = 0; n < i.operands; n++) {
		...
		if (operand_type_check(i.types[n], imm)) {
			if (i.op[n].imms->X_op == O_constant) {
				...
				if (i.types[n].bitfield.imm32s && (i.suffix == QWORD_MNEM_SUFFIX || (!i.suffix && i.tm.opcode_modifier.no_lsuf)))
					sign = 1;
				else
					sign = 0;
				...
				if ((reloc_type == BFD_RELOC_32 || reloc_type == BFD_RELOC_X86_64_32S || reloc_type == BFD_RELOC_64)
				    && GOT_symbol && GOT_symbol == i.op[n].imms->X_add_symbol && (i.op[n].imms->X_op == O_symbol || (i.op[n].imms->X_op == O_add && ((symbol_get_value_expression(i.op[n].imms->X_op_symbol)->X_op)
																				     == O_subtract)))) {
					offsetT add;
				...
				fix_new_exp(frag_now, p - frag_now->fr_literal, size, i.op[n].imms, 0, reloc_type);
			}
		}
	}
}
...
void x86_cons_fix_new(fragS * frag, unsigned int off, unsigned int len, expressionS * exp, bfd_reloc_code_real_type r) {
...
static char *lex_got(enum bfd_reloc_code_real *rel, int *adjust, i386_operand_type * types) {
...
#if defined (OBJ_ELF) || defined (OBJ_MAYBE_ELF)
		{
			STRING_COMMA_LEN("SIZE"), {
		BFD_RELOC_SIZE32, BFD_RELOC_SIZE32}, OPERAND_TYPE_IMM32_64},
#endif
		{
			STRING_COMMA_LEN("PLTOFF"), {
		_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_PLTOFF64}, OPERAND_TYPE_IMM64}, {
			STRING_COMMA_LEN("PLT"), {
		BFD_RELOC_386_PLT32, BFD_RELOC_X86_64_PLT32}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("GOTPLT"), {
		_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_GOTPLT64}, OPERAND_TYPE_IMM64_DISP64}, {
			STRING_COMMA_LEN("GOTOFF"), {
		BFD_RELOC_386_GOTOFF, BFD_RELOC_X86_64_GOTOFF64}, OPERAND_TYPE_IMM64_DISP64}, {
			STRING_COMMA_LEN("GOTPCREL"), {
		_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_GOTPCREL}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("TLSGD"), {
		BFD_RELOC_386_TLS_GD, BFD_RELOC_X86_64_TLSGD}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("TLSLDM"), {
		BFD_RELOC_386_TLS_LDM, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE}, {
			STRING_COMMA_LEN("TLSLD"), {
		_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_TLSLD}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("GOTTPOFF"), {
		BFD_RELOC_386_TLS_IE_32, BFD_RELOC_X86_64_GOTTPOFF}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("TPOFF"), {
		BFD_RELOC_386_TLS_LE_32, BFD_RELOC_X86_64_TPOFF32}, OPERAND_TYPE_IMM32_32S_64_DISP32_64}, {
			STRING_COMMA_LEN("NTPOFF"), {
		BFD_RELOC_386_TLS_LE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE}, {
			STRING_COMMA_LEN("DTPOFF"), {
		BFD_RELOC_386_TLS_LDO_32, BFD_RELOC_X86_64_DTPOFF32}, OPERAND_TYPE_IMM32_32S_64_DISP32_64}, {
			STRING_COMMA_LEN("GOTNTPOFF"), {
		BFD_RELOC_386_TLS_GOTIE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE}, {
			STRING_COMMA_LEN("INDNTPOFF"), {
		BFD_RELOC_386_TLS_IE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE}, {
			STRING_COMMA_LEN("GOT"), {
		BFD_RELOC_386_GOT32, BFD_RELOC_X86_64_GOT32}, OPERAND_TYPE_IMM32_32S_64_DISP32}, {
			STRING_COMMA_LEN("TLSDESC"), {
		BFD_RELOC_386_TLS_GOTDESC, BFD_RELOC_X86_64_GOTPC32_TLSDESC}, OPERAND_TYPE_IMM32_32S_DISP32}, {
			STRING_COMMA_LEN("TLSCALL"), {
	BFD_RELOC_386_TLS_DESC_CALL, BFD_RELOC_X86_64_TLSDESC_CALL}, OPERAND_TYPE_IMM32_32S_DISP32},};
	char *cp;
	unsigned int j;
	...
	for (j = 0; j < ARRAY_SIZE(gotrel); j++) {
		...
		if (strncasecmp(cp + 1, gotrel[j].str, len) == 0) {
			...
			as_bad(_("@%s reloc is not supported with %d-bit output format"), gotrel[j].str, 1 << (5 + object_64bit));
			return NULL;
		}
	}
...
static char *lex_got(enum bfd_reloc_code_real *rel ATTRIBUTE_UNUSED, int *adjust ATTRIBUTE_UNUSED, i386_operand_type * types) {
	static const struct {
		const char *str;
		int len;
		const enum bfd_reloc_code_real rel[2];
		...
		{
			STRING_COMMA_LEN("SECREL32"), {
	BFD_RELOC_32_SECREL, BFD_RELOC_32_SECREL}, OPERAND_TYPE_IMM32_32S_64_DISP32_64},};
...
bfd_reloc_code_real_type x86_cons(expressionS * exp, int size) {
	...
	if (size == 4 || (object_64bit && size == 8)) {
		...
		if (gotfree_input_line) {
			...
			input_line_pointer = (save + (input_line_pointer - gotfree_input_line)
					      + adjust);
			free(gotfree_input_line);
			if (exp->X_op == O_constant || exp->X_op == O_absent || exp->X_op == O_illegal || exp->X_op == O_register || exp->X_op == O_big) {
...
#ifdef TE_PE
static void pe_directive_secrel(int dummy ATTRIBUTE_UNUSED) {
	expressionS exp;
...
static char *check_VecOperations(char *op_string, char *op_end) {
	const reg_entry *mask;
	const char *saved;
	char *end_op;

	while (*op_string && (op_end == NULL || op_string < op_end)) {
		saved = op_string;
		if (*op_string == '{') {
			op_string++;
			...
			if (strncmp(op_string, "1to", 3) == 0) {
				...
				else if (*op_string == '1' && *(op_string + 1) == '6') {
			...
			/* Check masking operation.  */
				...
				if (mask->reg_num == 0) {
					as_bad(_("`%s' can't be used for write mask"), op_string);
					return NULL;
				}
 ...
 duplicated_vec_op:
					...
					/* Only "{%k}" is allowed here.  No need to check mask
					   register explicitly.  */
					if (i.mask->operand != this_operand) {
						as_bad(_("invalid zeroing-masking `%s'"), saved);
						return NULL;
					}
				}
...
static int i386_immediate(char *imm_start) {
	...
	if (i.imm_operands == MAX_IMMEDIATE_OPERANDS) {
		as_bad(_("at most %d immediate operands are allowed"), MAX_IMMEDIATE_OPERANDS);
		return 0;
	}
	...
	if (*input_line_pointer == '{') {
		input_line_pointer = check_VecOperations(input_line_pointer, NULL);
...
static int i386_finalize_immediate(segT exp_seg ATTRIBUTE_UNUSED, expressionS * exp, i386_operand_type types, const char *imm_start) {
	if (exp->X_op == O_absent || exp->X_op == O_illegal || exp->X_op == O_big) {
		if (imm_start)
			as_bad(_("missing or invalid immediate expression `%s'"), imm_start);
		return 0;
	} else if (exp->X_op == O_constant) {
		/* Size it properly later.  */
		i.types[this_operand].bitfield.imm64 = 1;
		/* If not 64bit, sign extend val.  */
		if (flag_code != CODE_64BIT && (exp->X_add_number & ~(((addressT) 2 << 31) - 1)) == 0)
			exp->X_add_number = (exp->X_add_number ^ ((addressT) 1 << 31)) - ((addressT) 1 << 31);
	}
#if (defined (OBJ_AOUT) || defined (OBJ_MAYBE_AOUT))
	else if (OUTPUT_FLAVOR == bfd_target_aout_flavour && exp_seg != absolute_section && exp_seg != text_section && exp_seg != data_section && exp_seg != bss_section && exp_seg != undefined_section && !bfd_is_com_section(exp_seg)) {
		...
		i.types[this_operand].bitfield.imm64 = 1;
		i.types[this_operand] = operand_type_and(i.types[this_operand], types);
	}

	return 1;
}

static char *i386_scale(char *scale) {
	offsetT val;
	...
	if (i.log2_scale_factor != 0 && i.index_reg == 0) {
		as_warn(_("scale factor of %d without an index register"), 1 << i.log2_scale_factor);
		i.log2_scale_factor = 0;
	}
...
static int i386_displacement(char *disp_start, char *disp_end) {
	expressionS *exp;
	segT exp_seg = 0;
	char *save_input_line_pointer;
	char *gotfree_input_line;
	...
	if (i.disp_operands == MAX_MEMORY_OPERANDS) {
		as_bad(_("at most %d displacement operands are allowed"), MAX_MEMORY_OPERANDS);
		return 0;
	}

	operand_type_set(&bigdisp, 0);
	if ((i.types[this_operand].bitfield.jumpabsolute)
	    || (!current_templates->start->opcode_modifier.jump && !current_templates->start->opcode_modifier.jumpdword)) {
		...
		if (flag_code == CODE_64BIT) {
			...
			if (!override)
				override = (i.suffix == (flag_code != CODE_16BIT ? WORD_MNEM_SUFFIX : LONG_MNEM_SUFFIX));
			bigdisp.bitfield.disp32 = 1;
	...
	i.types[this_operand] = operand_type_or(i.types[this_operand], bigdisp);
...
#define GCC_ASM_O_HACK 0
	...
	if (i.types[this_operand].bitfield.baseIndex && displacement_string_end[-1] == '+') {
...
static int i386_finalize_displacement(segT exp_seg ATTRIBUTE_UNUSED, expressionS * exp, i386_operand_type types, const char *disp_start) {
	...
	if (i.reloc[this_operand] == BFD_RELOC_386_GOTOFF || i.reloc[this_operand] == BFD_RELOC_X86_64_GOTPCREL || i.reloc[this_operand] == BFD_RELOC_X86_64_GOTOFF64) {
		if (exp->X_op != O_symbol)
			goto inv_disp;

		if (S_IS_LOCAL(exp->X_add_symbol)
		    && S_GET_SEGMENT(exp->X_add_symbol) != undefined_section && S_GET_SEGMENT(exp->X_add_symbol) != expr_section)
	...
	else if (exp->X_op == O_absent || exp->X_op == O_illegal || exp->X_op == O_big) {
 inv_disp:
		as_bad(_("missing or invalid displacement expression `%s'"), disp_start);
		ret = 0;
	}

	else if (flag_code == CODE_64BIT && !i.prefix[ADDR_PREFIX]
		 && exp->X_op == O_constant) {
		...
		if (!fits_in_signed_long(exp->X_add_number)) {
			...
			if (i.types[this_operand].bitfield.baseindex) {
				as_bad(_("0x%lx out range of signed 32bit displacement"), (long)exp->X_add_number);
				ret = 0;
			}
		}
	}
#if (defined (OBJ_AOUT) || defined (OBJ_MAYBE_AOUT))
	else if (exp->X_op != O_constant && OUTPUT_FLAVOR == bfd_target_aout_flavour && exp_seg != absolute_section && exp_seg != text_section && exp_seg != data_section && exp_seg != bss_section && exp_seg != undefined_section && !bfd_is_com_section(exp_seg)) {
		as_bad(_("unimplemented segment %s in operand"), exp_seg->name);
		ret = 0;
	}
#endif
...
static int i386_index_check(const char *operand_string) {
	...
	if (i.prefix[ADDR_PREFIX])
		...
		if (i.mem_operands == 0) {
			...
			if (addr_reg) {
				if (addr_reg->reg_num == RegEip || addr_reg->reg_num == RegEiz || addr_reg->reg_type.bitfield.reg32)
					addr_mode = CODE_32BIT;
				else if (flag_code != CODE_64BIT && addr_reg->reg_type.bitfield.reg16)
				...
				if (addr_mode != flag_code) {
					...
					if (flag_code != CODE_64BIT && (i.types[this_operand].bitfield.disp16 || i.types[this_operand].bitfield.disp32))
	...
	if (current_templates->start->opcode_modifier.isstring && !current_templates->start->opcode_modifier.immext && (current_templates->end[-1].opcode_modifier.isstring || i.mem_operands)) {
		...
		if (current_templates->start->opcode_modifier.w) {
			...
			if (!type.bitfield.baseindex || ((!i.mem_operands != !intel_syntax)
							 && current_templates->end[-1].operand_types[1]
							 .bitfield.baseindex))
				type = current_templates->end[-1].operand_types[1];
			expected_reg = hash_find(reg_hash, di_si[addr_mode][type.bitfield.esseg]);

		} else
			expected_reg = hash_find(reg_hash, bx[addr_mode]);

		if (i.base_reg != expected_reg || i.index_reg || operand_type_check(i.types[this_operand], disp)) {
			/* The second memory operand must have the same size as
			   the first one.  */
			if (i.mem_operands && i.base_reg && !((addr_mode == CODE_64BIT && i.base_reg->reg_type.bitfield.reg64)
							      || (addr_mode == CODE_32BIT ? i.base_reg->reg_type.bitfield.reg32 : i.base_reg->reg_type.bitfield.reg16)))
				goto bad_address;

			as_warn(_("`%s' is not valid here (expected `%c%s%s%c')"), operand_string, intel_syntax ? '[' : '(', register_prefix, expected_reg->reg_name, intel_syntax ? ']' : ')');
			return 1;
		} else
			return 1;

 bad_address:
		as_bad(_("`%s' is not a valid %s expression"), operand_string, kind);
		return 0;
	} else {
		if (addr_mode != CODE_16BIT) {
			/* 32-bit/64-bit checks.  */
			if ((i.base_reg && (addr_mode == CODE_64BIT ? !i.base_reg->reg_type.bitfield.reg64 : !i.base_reg->reg_type.bitfield.reg32)
			     && (i.index_reg || (i.base_reg->reg_num != (addr_mode == CODE_64BIT ? RegRip : RegEip))))
			    || (i.index_reg && !i.index_reg->reg_type.bitfield.regxmm && !i.index_reg->reg_type.bitfield.regymm && !i.index_reg->reg_type.bitfield.regzmm && ((addr_mode == CODE_64BIT ? !(i.index_reg->reg_type.bitfield.reg64 || i.index_reg->reg_num == RegRiz)
																					       : !(i.index_reg->reg_type.bitfield.reg32 || i.index_reg->reg_num == RegEiz))
																					      || !i.index_reg->reg_type.bitfield.baseindex)))
				goto bad_address;
		} else {
			/* 16-bit checks.  */
			if ((i.base_reg && (!i.base_reg->reg_type.bitfield.reg16 || !i.base_reg->reg_type.bitfield.baseindex))
			    || (i.index_reg && (!i.index_reg->reg_type.bitfield.reg16 || !i.index_reg->reg_type.bitfield.baseindex || !(i.base_reg && i.base_reg->reg_num < 6 && i.index_reg->reg_num >= 6 && i.log2_scale_factor == 0))))
				goto bad_address;
		}
	}
	return 1;
...
static int RC_SAE_immediate(const char *imm_start) {
...
static int i386_att_operand(char *operand_string) {
	const reg_entry *r;
	char *end_op;
	...
	if ((r = parse_register(op_string, &end_op)) != NULL) {
		...
		if (is_space_char(*op_string))
			++op_string;
		if (*op_string == ':' && (r->reg_type.bitfield.sreg2 || r->reg_type.bitfield.sreg3)) {
			switch (r->reg_num) {
			case 0:
 ...
 do_memory_reference:
		if ((i.mem_operands == 1 && !current_templates->start->opcode_modifier.isstring)
		    || i.mem_operands == 2) {
			as_bad(_("too many memory references for `%s'"), current_templates->start->name);
			return 0;
		}
		...
		if (*base_string == ')') {
			...
			if (*base_string == ',' || ((i.base_reg = parse_register(base_string, &end_op))
						    != NULL)) {
				...
				if (*base_string == ',') {
					...
					if ((i.index_reg = parse_register(base_string, &end_op))
						...
						if (*base_string == ',') {
							...
							as_bad(_("expecting `,' or `)' " "after index register in `%s'"), operand_string);
							return 0;
						}
					...
					if (*base_string != ')') {
						...
						if (is_space_char(*base_string))
							++base_string;
						if (*base_string != ')') {
							as_bad(_("expecting `)' " "after scale factor in `%s'"), operand_string);
							return 0;
						}
					} else if (!i.index_reg) {
						as_bad(_("expecting index register or scale factor " "after `,'; got '%c'"), *base_string);
						return 0;
					}
				} else if (*base_string != ')') {
					as_bad(_("expecting `,' or `)' " "after base register in `%s'"), operand_string);
					return 0;
				}
			} else if (*base_string == REGISTER_PREFIX) {
				end_op = strchr(base_string, ',');
		...
		if (i.base_reg && operand_type_equal(&i.base_reg->reg_type, &reg16_inoutportreg)
		    && i.index_reg == 0 && i.log2_scale_factor == 0 && i.seg[i.mem_operands] == 0 && !operand_type_check(i.types[this_operand], disp)) {
		...
		as_bad(_("invalid char %s beginning operand %d `%s'"), output_invalid(*op_string), this_operand + 1, op_string);
		return 0;
	}
...
unsigned int i386_frag_max_var(fragS * frag) {
...
int md_estimate_size_before_relax(fragS * fragP, segT segment) {
	...
	if (S_GET_SEGMENT(fragP->fr_symbol) != segment
#if defined (OBJ_ELF) || defined (OBJ_MAYBE_ELF)
	    || (IS_ELF && (S_IS_EXTERNAL(fragP->fr_symbol)
			   || S_IS_WEAK(fragP->fr_symbol)
			   || ((symbol_get_bfdsym(fragP->fr_symbol)->flags & BSF_GNU_INDIRECT_FUNCTION))))
#endif
#if defined (OBJ_COFF) && defined (TE_PE)
	    || (OUTPUT_FLAVOR == bfd_target_coff_flavour && S_IS_WEAK(fragP->fr_symbol))
#endif
	    ) {
		...
		case UNCOND_JUMP:
			...
			opcode[0] = 0xe9;
			fragP->fr_fix += size;
			fix_new(fragP, old_fr_fix, size, fragP->fr_symbol, fragP->fr_offset, 1, reloc_type);
			break;

		case COND_JUMP86:
			if (size == 2 && (!no_cond_jump_promotion || fragP->fr_var != NO_RELOC)) {
				...
				/* We added two extra opcode bytes, and have a two byte
				   offset.  */
				fragP->fr_fix += 2 + 2;
				fix_new(fragP, old_fr_fix + 2, 2, fragP->fr_symbol, fragP->fr_offset, 1, reloc_type);
				break;
			}
			/* Fall through.  */
		...
		case COND_JUMP:
			if (no_cond_jump_promotion && fragP->fr_var == NO_RELOC) {
				...
				fragP->fr_fix += 1;
				fixP = fix_new(fragP, old_fr_fix, 1, fragP->fr_symbol, fragP->fr_offset, 1, BFD_RELOC_8_PCREL);
				fixP->fx_signed = 1;
				break;
			}
			...
			fragP->fr_fix += 1 + size;
			fix_new(fragP, old_fr_fix + 1, size, fragP->fr_symbol, fragP->fr_offset, 1, reloc_type);
			break;
...
void md_convert_frag(bfd * abfd ATTRIBUTE_UNUSED, segT sec ATTRIBUTE_UNUSED, fragS * fragP) {
	...
	if ((fragP->fr_subtype & BIG) == 0) {
		...
		if (no_cond_jump_promotion && TYPE_FROM_RELAX_STATE(fragP->fr_subtype) != UNCOND_JUMP)
	...
	/* If size if less then four we are sure that the operand fits,
	   ...
	   then -/+ 2GB.  */
	if (DISP_SIZE_FROM_RELAX_STATE(fragP->fr_subtype) == 4 && object_64bit && ((addressT) (displacement_from_opcode_start - extension + ((addressT) 1 << 31))
										   > (((addressT) 2 << 31) - 1))) {
		as_bad_where(fragP->fr_file, fragP->fr_line, _("jump target out of range"));
		/* Make us emit 0.  */
		displacement_from_opcode_start = extension;
	}
	/* Now put displacement after opcode.  */
	md_number_to_chars((char *)where_to_put_displacement, (valueT) (displacement_from_opcode_start - extension), DISP_SIZE_FROM_RELAX_STATE(fragP->fr_subtype));
...
void md_apply_fix(fixS * fixP, valueT * valP, segT seg ATTRIBUTE_UNUSED) {
	...
	if (fixP->fx_addsy != NULL && (fixP->fx_r_type == BFD_RELOC_32_PCREL || fixP->fx_r_type == BFD_RELOC_64_PCREL || fixP->fx_r_type == BFD_RELOC_16_PCREL || fixP->fx_r_type == BFD_RELOC_8_PCREL)
		...
		if (IS_ELF) {
			...
			if ((sym_seg == seg || (symbol_section_p(fixP->fx_addsy)
	...
	if (fixP->fx_addsy != NULL && S_IS_WEAK(fixP->fx_addsy)
		...
		/* For x86 PE weak function symbols are neither PC-relative
		   ...
		   section.  */
		if (S_GET_SEGMENT(fixP->fx_addsy) != NULL && S_GET_SEGMENT(fixP->fx_addsy)->flags & SEC_CODE) ;
		else
#endif
...
static char *output_invalid(int c) {
	if (ISPRINT(c))
		snprintf(output_invalid_buf, sizeof(output_invalid_buf), "'%c'", c);
	else
		snprintf(output_invalid_buf, sizeof(output_invalid_buf), "(0x%x)", (unsigned char)c);
	return output_invalid_buf;
}

/* REG_STRING starts *before* REGISTER_PREFIX.  */

static const reg_entry *parse_real_register(char *reg_string, char **end_op) {
	char *s = reg_string;
	char *p;
	...
	*end_op = s;
	...
	if ((r->reg_type.bitfield.reg32 || r->reg_type.bitfield.sreg3 || r->reg_type.bitfield.control || r->reg_type.bitfield.debug || r->reg_type.bitfield.test)
	    && !cpu_arch_flags.bitfield.cpui386)
		return (const reg_entry *)NULL;

	if (r->reg_type.bitfield.floatreg && !cpu_arch_flags.bitfield.cpu8087 && !cpu_arch_flags.bitfield.cpu287 && !cpu_arch_flags.bitfield.cpu387)
		return (const reg_entry *)NULL;
	...
	if (!allow_index_reg && (r->reg_num == RegEiz || r->reg_num == RegRiz))
		return (const reg_entry *)NULL;

	/* Upper 16 vector register is only available with VREX in 64bit
	   mode.  */
	if ((r->reg_flags & RegVRex)) {
		if (!cpu_arch_flags.bitfield.cpuvrex || flag_code != CODE_64BIT)
			return (const reg_entry *)NULL;
	...
	if (((r->reg_flags & (RegRex64 | RegRex))
	    ...
	    && (!cpu_arch_flags.bitfield.cpulm || !operand_type_equal(&r->reg_type, &control))
	    && flag_code != CODE_64BIT)
		return (const reg_entry *)NULL;
...
static const reg_entry *parse_register(char *reg_string, char **end_op) {
	const reg_entry *r;
	...
	if (!r) {
		...
		if (symbolP && S_GET_SEGMENT(symbolP) == reg_section) {
			...
			know(e->X_add_number >= 0 && (valueT) e->X_add_number < i386_regtab_size);
...
int i386_parse_name(char *name, expressionS * e, char *nextcharP) {
	const reg_entry *r;
	...
	case OPTION_64:
		{
			...
			for (l = list; *l != NULL; l++)
				if (CONST_STRNEQ(*l, "elf64-x86-64")
				    || strcmp(*l, "coff-x86-64") == 0 || strcmp(*l, "pe-x86-64") == 0 || strcmp(*l, "pei-x86-64") == 0 || strcmp(*l, "mach-o-x86-64") == 0) {
					default_arch = "x86_64";
					break;
				}
			if (*l == NULL)
		...
		do {
			...
			for (j = 0; j < ARRAY_SIZE(cpu_arch); j++) {
				if (strcmp(arch, cpu_arch[j].name) == 0) {
					...
					break;
				} else if (*cpu_arch[j].name == '.' && strcmp(arch, cpu_arch[j].name + 1) == 0) {
					/* ISA entension.  */
					i386_cpu_flags flags;
...
static void show_arch(FILE * stream, int ext, int check) {
...
const char *i386_target_format(void) {
	if (!strncmp(default_arch, "x86_64", 6)) {
...
void i386_elf_emit_arch_note(void) {
	if (IS_ELF && cpu_arch_name != NULL) {
		...
		bfd_set_section_flags(stdoutput, note_secp, SEC_HAS_CONTENTS | SEC_READONLY);
...
symbolS *md_undefined_symbol(char *name) {
	if (name[0] == GLOBAL_OFFSET_TABLE_NAME[0]
		...
		if (!GOT_symbol) {
			...
			GOT_symbol = symbol_new(name, undefined_section, (valueT) 0, &zero_address_frag);
		};
		return GOT_symbol;
	}
	return 0;
...
void i386_validate_fix(fixS * fixp) {
...
arelent *tc_gen_reloc(asection * section ATTRIBUTE_UNUSED, fixS * fixp) {
	...
	case BFD_RELOC_SIZE64:
		if (S_IS_DEFINED(fixp->fx_addsy)
			...
			if (fixp->fx_r_type == BFD_RELOC_SIZE32 && !fits_in_unsigned_long(value))
				as_bad_where(fixp->fx_file, fixp->fx_line, _("symbol size computation overflow"));
			fixp->fx_addsy = NULL;
			fixp->fx_subsy = NULL;
	...
	default:
		if (fixp->fx_pcrel) {
			switch (fixp->fx_size) {
			default:
				as_bad_where(fixp->fx_file, fixp->fx_line, _("can not do %d byte pc-relative relocation"), fixp->fx_size);
			...
			switch (fixp->fx_size) {
			default:
				as_bad_where(fixp->fx_file, fixp->fx_line, _("can not do %d byte relocation"), fixp->fx_size);
				code = BFD_RELOC_32;
				break;
			case 1:
				code = BFD_RELOC_8;
	...
	*rel->sym_ptr_ptr = symbol_get_bfdsym(fixp->fx_addsy);
	...
	/* Use the rela in 64bit mode.  */
		...
		if (disallow_64bit_reloc)
			...
			case BFD_RELOC_X86_64_GOTPLT64:
			case BFD_RELOC_X86_64_PLTOFF64:
				as_bad_where(fixp->fx_file, fixp->fx_line, _("cannot represent relocation type %s in x32 mode"), bfd_get_reloc_code_name(code));
				break;
			default:
				break;
			}
		...
		if (!fixp->fx_pcrel)
			...
			case BFD_RELOC_X86_64_TLSDESC_CALL:
				...
				break;
			default:
				rel->addend = (section->vma - fixp->fx_size + fixp->fx_addnumber + md_pcrel_from(fixp));
				break;
			}
	}
	...
	if (rel->howto == NULL) {
		as_bad_where(fixp->fx_file, fixp->fx_line, _("cannot represent relocation type %s"), bfd_get_reloc_code_name(code));
...
void tc_x86_parse_to_dw2regnum(expressionS * exp) {
	int saved_naked_reg;
...
int i386_elf_section_type(const char *str, size_t len) {
	if (flag_code == CODE_64BIT && len == sizeof("unwind") - 1 && strncmp(str, "unwind", 6) == 0)
...
#ifdef TE_SOLARIS
void i386_solaris_fix_up_eh_frame(segT sec) {
	if (flag_code == CODE_64BIT)
...
#ifdef TE_PE
void tc_pe_dwarf2_emit_offset(symbolS * symbol, unsigned int size) {
	expressionS exp;
...
bfd_vma x86_64_section_letter(int letter, char **ptr_msg) {
	if (flag_code == CODE_64BIT) {
		if (letter == 'l')
...
bfd_vma x86_64_section_word(char *str, size_t len) {
